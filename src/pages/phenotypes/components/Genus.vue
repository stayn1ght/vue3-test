<template>
  <div class="associated-genus">
    <div class="info">
      <p>
        In total
        <span class="text-red-800 font-bold italic">{{ total.toLocaleString() }}</span> genera were
        found in samples of this phenotype collected in our database.
        <my-link
          :to="`https://gmrepo.humangut.info/Downloads/SpeciesAndGeneraAssociatedWithPhenotypeID/genus_associated_with_${id}.tsv.gz`"
          is-external
          :show-icon="false"
          >Download stats on all genera as tsv
      ...
      <dl>
        <dt>
          Shown below are
          <span class="text-red-800 font-bold italic">{{ genus.length.toLocaleString() }}</span>
          genera presented
        </dt>
        <dd>in >=2 samples</dd>
    ...
    <my-link
      :show-icon="false"
      :to="`https://gmrepo.humangut.info/Downloads/CoocurrenceByPhenotypeID/genus_cooccurrence_in_${id}.tsv.gz`"
      is-external
    >
...
<script lang="ts" setup>
  ...
  const props = defineProps<{
    data: associated_genusItem[]
    total: number
    id: string
    d3Data: networkdata_d3_genusItem
    term: any
    pairTotal: any
    num: any
  }>()
  ...
  const tableHead = [
    { label: 'Genus', slotName: 'genus', prop: 'scientific_name' },
    {
  ...
  const table = reactive({
    data: [] as associated_genusItem[],
    total: 0,
    page: 1,
    limit: 10,
    keywords: '',
    totalData: [] as associated_genusItem[],
    except: false
  })
  const { handleSortChange, searchTable, refreshSearch, changePage } =
    useTableTools<associated_genusItem>(genus, table)
  watch(
    () => props.data,
    (val) => {
      genus.value = val
  ...
  const handleGenusChange = (val: string) => {
