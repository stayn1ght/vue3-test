...
<script setup lang="ts">
  ...
  import { Genus } from '@api/taxon/types'
  ...
  const handleSortChange = ({ prop, order }: any) => {
    table.totalData.sort((a: Genus, b: Genus) => {
      if (order === 'descending') {
        // @ts-ignore
  ...
  const tableHead = [
    { label: 'Genus', slotName: 'genus', prop: 'name' },
    {
  ...
  const searchTable = () => {
    ...
    table.page = 1
    table.totalData = props.genus.filter((item: Genus) => {
  ...
  const expandChange = (row: Genus, expandedRows: any) => {
  ...
  const layout1 = {
    ...
    title: 'Genus prevalence in <b>phenotypes</b>',
    xaxis: {
      title: 'Nr. phenotypes in which a genus is found'
    },
    yaxis: {
  ...
  const layout2 = {
    ...
    title: 'Genus prevalence in <b>samples</b>',
    xaxis: {
      title: 'Nr. samples in which a genus is found (log-transformed)'
    },
    yaxis: {
      title: 'Proportion of genera'
    },
    legend: {
      x: 0.5,
      y: 1,
  ...
  const setPlotData = () => {
    ...
    for (let i = 0; i < props.genus.length; i++) {
      densitySpeciesPhenotypes.x.push(props.genus[i].nr_phenotypes)
      densitySpeciesSamples.x.push(props.genus[i].presented_samples)
    }
